(() => {
  ...
  const nodeDetails = {
    ...
    "module-detail-design-split": {
      ...
      output: "module_groups 数据：组名、模块列表和需求短名。",
    ...
    "module-asis-analysis": {
      ...
      output: "<AR>-<需求短名>-<模块组>模块详细设计说明书.context.md（必需）。",
    ...
    "module-tobe-design": {
      ...
      input: "当前模块组的 AS-IS context.md。",
      action: "定义目标结构、接口、数据、流程、兼容策略、风险与工程落点；每项决策引用对应事实。",
      output: "<AR>-<需求短名>-<模块组>模块详细设计说明书.md（必需）。",
    ...
    "module-test-design": {
      ...
      output: "<AR>-<需求短名>-<模块组>模块测试用例设计.md（必需）。",
    ...
    "module-design-gate": {
      ...
      output: "<AR>-<需求短名>-<模块组>模块设计门禁结果.md（必需）。",
    ...
    "task-split": {
      ...
      output: "<模块组>_tasks/overview.md；不生成独立 T<N> 任务文件。",
