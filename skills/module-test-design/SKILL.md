...
- 输入或触发步骤。
- 预期结果、断言方式和可观察信号。断言必须写到能直接翻译为测试代码的程度——写明对哪个变量做什么比较、期望什么值，不得只写"验证返回值正确""检查状态变化"等分类概要。
...
由于测试层级已体现在顶层分组标题，单条用例条目内不再重复”测试层级”字段；每条用例用列表逐项列出优先级、覆盖目标、场景、建议位置、自动化，以及四个行为字段「前置 / 输入 / 预期 / 断言」（两两对仗：前置—输入描述怎么跑，预期—断言描述怎么验）。
