...
version: "2.3.2.4"
description: 把已定稿的设计目标、契约、流程、异常边界、风险和可测试性输入转化为可执行的最小充分验证集，输出独立测试用例设计文档。本 skill 只输出最小充分测试用例集、覆盖矩阵、断言、建议位置和缺口；不得替设计补内容，不得拆分 AICoding 任务。支持两种模式：严格模式（SR/AR 入口）基于模块目录下的正式《模块详细设计说明书.md》与 `.context/详细设计上下文.md`，生成同目录《模块测试用例设计.md》；轻量模式（dev 入口）基于单份 `dev-design.md`，生成 `.sdd/{SR}/test-design.md`，由工作单显式声明。
---
...
- **严格模式（默认）**：SR/AR 入口调用。权威输入是《模块详细设计说明书》与 `.context/详细设计上下文.md`。按下方正文执行，输入缺失时按阻塞规则处理。
- **轻量模式**：dev 入口调用。工作单的 `prompt` 字段会显式声明"本实例为 module-test-design 的【轻量模式】"。此时权威输入只有 `.sdd/{SR}/dev-design.md`，不存在三件套，**也不要求它们**。按「轻量模式分支」执行，不要因为缺三件套而中止或引导用户跑 module-tobe-design / module-design-gate。
...
- 输入正式说明书：`.sdd/{SR}/{AR}/{模块组名}/模块详细设计说明书.md`。
- 输入过程上下文：`.sdd/{SR}/{AR}/{模块组名}/.context/详细设计上下文.md`。
- 输出测试设计：`.sdd/{SR}/{AR}/{模块组名}/模块测试用例设计.md`。
...
- **本集合是下限，不是上限**：设计期只能看到设计面，实现期才会暴露真正的测试面（生命周期、边界行为、配置归一化等）。实现阶段发现的补充验证面由 `task-dev` 在实现期补测，并在 tasks-overview 执行记录的「实现期补充与残余风险」小节登记，不要求回流本文件。
...
| 输入 | 默认路径 | 要求 |
|---|---|---|
| 轻量设计文档 | `.sdd/{SR}/dev-design.md` | 必需。方案与契约变更章节是被验证设计的事实来源；验收标准章节是需求侧验收口径 |
| 轻量门禁结果 | `.sdd/{SR}/.context/dev-design-gate.md` | 可选。存在时参考其历史问题 |
| 软件架构 | `.sdd/software_architecture.md` | 可选 |

启动后校验：`dev-design.md` 真实存在且可读；其方案与契约变更足以支撑测试设计，验收标准覆盖需求的全部目标。**不要**校验三件套文档，**不要**因其缺失而中止。

`dev-design.md` 缺失，或验收/契约不足以写出输入、预期结果、断言时，按"不能验证的设计就是设计缺口"输出缺口并回流 `dev-design`；不得硬凑用例，也不得替设计补充错误码、字段语义或兼容策略。
...
| 验收口径来源 | TOBE 可测试性与验收口径 | `dev-design.md` 的验收标准章节（需求侧口径） |
| 输出路径 | `.sdd/{SR}/{AR}/{模块组名}/模块测试用例设计.md` | `.sdd/{SR}/test-design.md` |
| 模板 | `references/test-design-template.md` | `references/test-design-lite-template.md` |
| 覆盖矩阵行 | TOBE 决策/契约/风险 | 验收标准条目 + 契约变更项 |
| 「首次可验证阶段」列 | 无 | 预留：设计时空着，由 task-split 回填 |

### 轻量模式附加要求

- **引用而不复述**：用例通过 `dev-design.md` 的契约项名称、章节锚点或验收标准原文引用设计，不复述设计内容——两份文档各管一头，防止漂移。
- **覆盖矩阵必须含「首次可验证阶段」列**：设计阶段留空；只有端到端才能验证的条目在该列标"端到端（末阶段）"。`task-split` 拆分任务时回填阶段归属，并在 `tasks-overview.md` 为每个任务圈定负责的用例 ID。
- **测试层级按项目实情裁剪**：依据仓库现有测试组织确定可支撑的层级；没有测试框架时提供可执行的手动验证步骤并说明原因。
- 交付对象为下游 `dev-design-gate`（验收闭环检查）与 `dev-task-split`（阶段回填与任务圈定）。

除输入来源、输出路径、模板和覆盖矩阵形态外，其余——测试设计哲学、内部工作法、阻塞规则、质量标准——全部沿用本 skill 正文。
...
- 定位同一模块目录下的正式说明书、`.context/详细设计上下文.md` 和目标测试设计文件。
...
测试用例按**测试层级分层组织**（对应模板的 `### 3.x` 顶层分组），本次设计能支撑的层级才建，不强求枚举：
...
- 最小充分测试用例集：用例正文采用列表式分层呈现——顶层按测试层级分组（`### 3.x`，只建本次能支撑的层级）、层级下按功能点/场景分组（`#### 3.x.y`）、每个用例一个条目（`##### TCn`）用列表逐项列出字段。单条用例写清优先级、覆盖目标、场景、建议位置、自动化，以及前置 / 输入 / 预期 / 断言四个行为字段。**用例正文不得使用任何表格**；测试层级不作为单条用例的字段，只作为顶层分组标题。
