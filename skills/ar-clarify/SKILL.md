...
name: ar-clarify
description: >
  从 SR 设计文档中提取指定 AR 的所有相关内容，从代码实现角度审视并澄清未覆盖的细节，
  生成独立的 AR 范围文档作为后续详细设计和开发的唯一输入。
triggers:
  - pattern: |
      (?:澄清|细化|审视|检查|review|分析|提取).*(?:AR|分配需求|需求条目)
    description: 用户想要对 SR 设计文档中的某条 AR 进行实现层面的澄清。
---
...
- **SR 设计文档**：由 `sr-design` Skill 生成的功能/模块设计文档，包含需求背景、功能设计、AR 拆分与交互定义、需求追溯等章节。
- **AR（分配需求）**：Allocated Requirement，SR 设计文档中"AR 拆分与交互定义"章节的条目，每个 AR 描述了一个承接模块需要实现的具体需求。AR 以名称作为唯一标识，无编号。
...
向用户询问："请提供 SR 设计文档的路径，以及要澄清的 AR 名称。我将从 SR 设计文档中提取该 AR 的相关信息。"

用户提供后，读取 SR 设计文档。定位该 AR 在文档中的所有相关内容：
...
记录该 AR 涉及的所有章节，后续在第 4 步中将这些章节的内容完整复制到 AR 范围文档中。
...
- **SR 内容完整复制**：将第 1 步中记录的该 AR 涉及的所有 SR 文档章节内容，按模板章节结构原封不动地复制到对应位置。不得改写为摘要，不得删减。
...
按模板输出完整文档，写入文件。文件名为 `AR-clarify.md`，存放在 SR 设计文档同目录下。

文档写入后，先进行自查再进入审核循环：

**自查**：重新梳理 SR 设计文档中该 AR 的所有相关内容、澄清过程中确认的所有答案，逐项对照生成的文档，检查是否存在以下遗漏：
- SR 设计文档中该 AR 的相关内容是否完整复制，没有被遗漏
- 澄清过程中确认的细节在文档中未写入
- 模板中的占位符未替换
- 模板中的章节有缺失
- 用例设计覆盖了四个维度
