...
每次只处理工作单指定的一个 Task。详细设计是实现设计的事实来源，测试用例设计是验证规格的事实来源；工作单 `input` 中 `artifact=task_overview` 的成果物只管理任务范围、顺序和最终交接。
...
→ 回填 task_overview 成果物、候选提交信息、done
```

### 1. 实现与测试

1. 按工作单 `input` 的精确路径完整读取 `task_overview`、`module_design`、`test_design`、`gate_result` 成果物和 `.sdd/software_architecture.md`，以及存在时的 `.sdd/spec.md`。不得自行拼接、替换或移动成果物路径。
...
先在工作单指定的 `task_overview` 成果物的 `## 执行记录` 中写入当前 Task 的最终交接：状态、修改文件、核心实现、设计偏差、待处理项、后续须知和 AAW 证据引用。不要写阶段勾选或完整日志。
