...
- ASIS 结论状态必须贯穿到 TOBE：TOBE 只能把已复核吸收的 ASIS 结论作为事实引用；引用 `推断`、`待确认`、低置信度或阻塞相关结论时，必须在详细设计上下文
...
- 对会跨章节反复出现的字段、DTO、接口、配置项、事件名、错误码、状态值和时间单位，必须在正式说明书的权威位置（4.1.4）先给出单点定义，并在摘要、图、表、工程落点、可测试性输入和风险章节通过契约编号引用同一名称、类型、单位和
...
接口和数据契约不属于实现越界：完整方法/函数签名、DTO/字段、序列化名称、错误码、状态、配置、表结构约束和必要的请求响应示例应按契约保真要求保留。只有当核心算法无法通过约束、决策表或流程说明准确表达时，才使用简短、语言无关且不可直接运行的伪代码。

当目标行为依赖核心算法时，必须讲清输入输出、前置条件、关键不变量、决策分支、终止条件、失败行为以及适用的数据规模或复杂度约束，不展开逐行实现。当关键设计模式实质影响职责分配、协作方式或扩展边界时，必须讲清它解决的问题、参与者职责、协作关系、真实扩展点和局限；只写“采用某模式”不构成完整设计。普通 CRUD、简单字段映射或局部调用不强求算法或设计模式说明。
...
下文 §1–§6 是每个环节的**操作细节**（怎么做）；本流程图是**整体骨架**（在做什么）。

### 1. 读取正式说明书和详细设计上下文

- 定位当前 SDD 工作流指定的 `.sdd/{SR}/{AR}/{模块组名}/模块详细设计说明书.md` 和同一模块目录下的 `.context/详细设计上下文.md`。
- 读取详细设计上下文中的关键 ASIS 事实（C3）、证据索引（C7）、模块边界（C2）、调用链与数据流（C4）、规格漂移与待确认（C6）和需求/AR 追溯矩阵（C8）。
- 读取 ASIS 变更类型；当变更类型为 `纯新增行为` 时，将“新增对象当前不存在”视为可承接的 ASIS 事实，而不是既有实现证据缺失。
- 读取上游设计、模块影响性分析、模块间交互设计和已有门禁整改项。
- 提取“关键契约清单”：从用户输入、上游设计、模块交互、ASIS context 和已有正式说明书中收集已经明确给出的接口、签名、DTO/字段、JSON
  名称、错误码、Topic/Path、配置项和数据结构。契约的**实质内容（字段、签名、入参出参等）写入 4.1.4**，第 8 章关键契约清单只登记编号、类型、名称和指向 4.1.4 的定义位置；来源位置和内部追踪写入详细设计上下文。
...
正式说明书必须按 `<skill-dir>/references/tobe-output-template.md` 的 9 个一级章节组织。第 4 章按需求点展开，是后续
...
|-----------------|-------|------------------------------------------------|
| 包/模块结构变化        | 是 / 否 | 包图、结构变化表、依赖方向和边界说明 → 4.1.4                       |
| 类/接口变化          | 是 / 否 | 类图、类/接口/抽象类清单、关键方法签名、职责和约束 → 4.1.4               |
| 业务流程变化          | 是 / 否 | 主流程图；异常、回滚、异步、状态变化按需展开 → 4.1.5                  |
| REST 接口变化       | 是 / 否 | Method、Path、认证/权限、请求、响应、错误码、兼容性 → 4.1.4（第 5 章只放索引）  |
| Kafka/MQ 接口变化   | 是 / 否 | Topic、Producer/Consumer、消息结构、Key、幂等、重试、死信、兼容策略 → 4.1.4（第 5 章只放索引） |
| 内部 Java 接口/抽象变化 | 是 / 否 | 调用方、实现方、方法签名、入参出参、异常语义、扩展约束 → 4.1.4（第 7 章只放概览）      |
...
根据正式大纲和变化触发表在正式说明书中输出。**接口/类/数据结构契约的权威定义位置是 4.1.4，流程/时序图的权威位置是 4.1.5**；第 5 章对外接口、第 7 章受影响模块与交互、第 8 章关键契约清单均为索引/概览，只引用权威位置，不复述字段细节、签名或图。

- 包/模块结构设计：只画本次变化相关包图，必须体现依赖方向、模块边界和防腐位置。→ 4.1.4
- 类与接口设计：只画关键类图，列出 Class、Interface、AbstractClass、Enum、DTO、Adapter
  等职责、关键方法、完整方法/函数签名、入参、出参、异常和设计约束。若上文已有明确签名，必须保留该签名并标注来源。→ 4.1.4
- 业务流程设计：输出主流程；涉及失败路径、回滚、事务、异步、状态变化时，补充异常与回退流程。→ 4.1.5
- 对外接口设计：REST 必须写 method、path、权限、请求体字段、响应体字段、错误码、示例和兼容性；Kafka/MQ 必须写
  topic、producer/consumer、消息结构字段、key、幂等、重试、死信和兼容策略。若上文已有明确接口契约，正式说明书不得省略任何字段。字段细节写 4.1.4；第 5 章对外接口只放 Method/Path/调用方/本次变化/关联契约索引。
- 内部接口与调用设计：写清调用方、实现方、方法/函数完整签名、每个入参和出参的类型与语义、异常语义和依赖方向；新增抽象必须说明扩展目的，禁止无真实扩展点的过度抽象。细节写 4.1.4；第 7 章受影响模块与交互只放概览表。
...
TOBE 定稿前，检查正式说明书中的关键设计变量是否一致。**一致性指同一字段/接口在各处定义不冲突，不要求在多处重写实质内容**——接口字段的实质内容只在 4.1.4 写一次，时序图只在 4.1.5 画一次，其他章节引用编号即可。

- 同一字段在前端类型、后端模型、JSON 示例、4.1.4 接口表、配置表、工程落点、可测试性输入和风险章节中使用同一类型、单位、默认值和兼容语义；其他章节只引用契约编号，不复述字段定义。
...
- 标准输出：保留核心骨架，展开与本次需求相关的设计点和风险专题。实质内容集中在 4.1.4 / 4.1.5 等权威位置，概览章只引用。
- 增强输出：补齐触发风险专题的专项分析和更严格验证设计。同样遵循权威位置约定，概览章不重复实质内容。
- 第 4 章必须按需求点展开
  `需求描述 / 现状约束摘要 / TOBE 设计说明 / 类接口数据结构变化 / 处理流程 / 异常回退边界场景 / 日志可观测性 / 可测试性与验收口径`。
