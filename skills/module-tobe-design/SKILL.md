...
- ASIS 结论状态必须贯穿到 TOBE：TOBE 只能把已复核吸收的 ASIS 结论作为事实引用；引用 `推断`、`待确认`、低置信度或阻塞相关结论时，必须在 `.context.md`
...
- 读取 `.context.md` 中的 ASIS 结论、需求/AR 与 ASIS 证据映射、ASIS 探索任务清单、探索任务结果汇总、复核与吸收记录、证据索引、待确认问题、ASIS 阻塞项。
...
- TOBE 要引用的 ASIS 事实是否已在复核与吸收记录中标记为已吸收或修正后吸收。
...
- 每条重要 TOBE 决策在正式说明书中有用户可读的设计依据摘要，并能在 `.context.md` 中反查需求/AR 来源、ASIS 探索任务、复核吸收记录和 ASIS 证据编号。
