...
如果当前环境提供 `ask_user` 工具，影响模块边界、核心方案、数据/权限/兼容策略、性能目标、验收阈值、测试设计输入或后续任务拆分的待确认项必须优先用 `ask_user` 发起问询，并在 `.context.md` 中记录问题、回答、影响范围和处理结论；正式说明书只承接已确认的设计结论或仍未关闭的待确认事项。工具不可用时，才退回当前会话问询或标记为 `需前置确认`。
...
- ASIS 结论状态必须贯穿到 TOBE：TOBE 只能把主 Agent 已复核吸收的 ASIS 结论作为事实引用；引用 `推断`、`待确认`、低置信度或阻塞相关结论时，必须在 `.context.md`
...
```mermaid
flowchart TD
    S1["§1 读取正式说明书 + .context.md<br/>ASIS结论/证据/变更类型"] --> S2["§2 初步对齐设计范围<br/>本模块承担/不承担/相邻配套 + 输出模式"]
    S2 --> S3{"§3 ASIS 覆盖充分?"}
    ...
    S5 --> S6["§6 更新成果物<br/>正式说明书 + .context.md"]
    S6 --> Q{"质量标准达标?"}
    Q -->|"否"| S4
...
- 读取 `.context.md` 中的 ASIS 结论、需求/AR 与 ASIS 证据映射、ASIS 探索任务清单、探索任务结果汇总、主 Agent 复核与吸收记录、证据索引、待确认问题、ASIS 阻塞项。
...
- TOBE 要引用的 ASIS 事实是否已在主 Agent 复核与吸收记录中标记为已吸收或修正后吸收。
- 关键结论是否为事实，还是推断/待确认/阻塞相关。
- 规格漂移、低置信度或测试缺口是否影响 TOBE 定稿。
- 纯新增场景下，ASIS 是否已确认模块边界，并提供新增对象不存在的检索证据、相邻同类实现或不存在相邻实现时可替代的模块惯例。

依赖 ASIS 阻塞项、低置信度结论、待确认关键事实、未完成关键探索任务或未经主 Agent 复核吸收的查证结果的 TOBE 决策不得定稿。只能继续设计与该阻塞无依赖的独立部分。
...
- 纯新增场景下，ASIS 未确认模块边界，或未提供新增对象不存在的检索证据、相邻同类实现或不存在相邻实现时可替代的模块惯例。
...
- 每条重要 TOBE 决策在正式说明书中有用户可读的设计依据摘要，并能在 `.context.md` 中反查需求/AR 来源、ASIS 探索任务、主 Agent 复核吸收记录和 ASIS 证据编号。
- 未完成、低置信度或未经主 Agent 复核吸收的 ASIS 探索结果没有被降级成 TOBE 事实。
- 设计没有越过模块职责边界。
