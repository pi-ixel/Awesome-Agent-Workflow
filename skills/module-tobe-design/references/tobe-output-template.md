...
TOBE 必须基于 `.context/详细设计上下文.md` 中的 ASIS 证据形成设计，不得脱离证据凭空设计。正式说明书是面向用户、评审和开发的标准交付件，只呈现设计结论、现状约束摘要、接口契约、模块边界、目标方案与实现约束、风险、可测试性输入和待确认事项；不得写入 ASIS 检索过程、证据编号表、推导过程、反证记录、门禁采样、追踪矩阵等过程性内容。详细设计上下文只记录证据、推导和过程，不参与后续开发指导。
...
补充 3.1 决策表中未展开的设计理由、与 AR/ASIS 假设的差异、关键约束的处理方式，以及影响用户判断的方案取舍。让读者能够回答“目标行为是否正确、边界是否合理、代价和风险是否可接受”；没有真实备选方案时不机械编造方案对比。不重复 3.1 已给出的决策结论——如果某条决策在 3.1 已经说清楚了，本节不再复述。

当本需求点存在决定行为正确性的核心算法时，说明输入输出、前置条件、关键不变量、决策分支、终止条件、失败行为和数据规模/复杂度约束，不展开逐行实现。当关键设计模式决定职责分配、运行时协作或扩展边界时，说明它解决的问题、参与者职责、协作关系、真实扩展点和局限；仅写模式名称无效。普通 CRUD、简单字段映射或局部调用不需要牵强补充算法或设计模式。
