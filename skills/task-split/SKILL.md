...
- **轻量模式**：dev 入口调用。工作单的 `prompt` 字段会显式声明“本实例为 task-split 的【轻量模式】”。此时权威输入只有 `.sdd/{SR}/dev-design.md`，不存在三件套，**也不要求它们**。按「轻量模式分支」执行，不要因为缺三件套而中止或引导用户跑 module-tobe-design / module-test-design / module-design-gate。
...
1. 解析 SR，定位 `.sdd/{SR}/dev-design.md`。
2. 完整读取该文档；存在 `.context/dev-design-gate.md` 时一并读取并确认结论为通过。
3. 提取方案、契约变更、影响文件（工程落点）和存疑项。
4. 提取验收标准条目，作为覆盖目标与验证规格。
5. 不校验、不读取、不要求三件套文档。
