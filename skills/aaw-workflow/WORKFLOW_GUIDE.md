...
CLI 中的 step 状态决定是否执行：只要 step 未完成且被 `next` 返回为 ready，就必须执行。已有 output 只表示存在一版可继续修改的成果，不能作为跳过 step、直接 `done` 的依据。重新执行时先读取并评估已有成果，复用仍有效的信息，可按当前要求局部修改或整体重写，并写回原路径。
