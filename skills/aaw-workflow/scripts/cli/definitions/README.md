...
跨节点复用的成果物必须在 input/output 中声明稳定的 `artifact` 标识。创建后继 step 时，CLI 按 `artifact` 和 SR/AR/模块组范围查找上游实际输出，并原样沿用已经登记的路径；尚未产生的 output 才按当前 definition 的路径模板创建。升级不会移动、复制或重命名已有文件，因此历史工作流可以安全地同时引用旧路径成果物和新路径成果物。Skill 和校验器必须使用工作单中的实际路径，不得根据文件名重新推断成果物。
