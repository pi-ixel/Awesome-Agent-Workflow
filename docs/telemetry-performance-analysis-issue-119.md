...
建议先消除 N+1 查询和分页前全量展开，再根据数据库容量配置连接池与 worker。nginx upstream keepalive 可以保留，但预期收益远低于查询优化。

## 证据边界

分析基于仓库提交 `9c22654` 和 `tools/profile_dashboard_queries.py` 的合成数据剖析。SQL 条数反映当前 ORM 执行路径；SQLite 耗时和按每条 SQL 3 ms 计算的等待时间仅用于展示增长趋势，不代表生产 MySQL 实测值。
...
在 500 个 Workflow 下，剖析脚本还对两个局部优化做了隔离实验：
...
实验只改变剖析脚本中的 QueryService 子类，不修改生产实现。它表明前两项修复可以先消除大部分查询，但要达到完整页面 50 条 SQL 以内，仍需批量加载当前页关联数据，并将主要聚合下推到数据库。

复现命令：

```powershell
cd telemetry-server
.venv\Scripts\python.exe tools/profile_dashboard_queries.py --workflows 500
...
### P0：CodeAttribution 懒加载形成 N+1 SQL
...
具体索引必须通过生产 MySQL 的执行计划验证，不能仅凭代码静态添加。

### P3：nginx upstream 未复用连接
...
1. 为 DevRun → CodeAttribution 使用批量预加载，消除归因 N+1 查询。
2. 将 workflows 的筛选、排序和分页下推到 SQL，再批量加载当前页 Message、DevRun 和 Attribution。
3. 消除前端重复 projects 请求；评估用单个聚合端点或服务端共享查询降低页面扇出。
4. 将 projects、users、trends、steps 等聚合逐步改为数据库端 `GROUP BY`，避免全量 ORM 对象加载和 Python 聚合。
5. 在生产 MySQL 上采集执行计划，再补充 `workflow_kind + started_at`、`workflow_kind + repository` 等候选索引。
6. 增加连接池 checkout 时间、SQL 数量、接口分段耗时和慢查询指标。
7. 根据数据库连接预算设置 worker 与连接池，满足：

   ```text
   workers × (pool_size + max_overflow) + 后台/运维保留连接 < MySQL max_connections
   ```

8. 确认后台任务的跨进程执行模型后，再决定是否增加 worker。
9. 最后补充 nginx upstream keepalive，并用同一基准验证收益。
