...
本分支已经消除 Attribution N+1、把工作流分页下推到 SQL 并批量加载当前页关联数据，同时合并重复 projects 请求。合成数据中，1,000 个 Workflow 的完整刷新从 9,025 条 SQL 降至 36 条。连接池上限也已配置化，并补充候选索引与 nginx upstream keepalive；生产 MySQL 的耗时和执行计划仍需上线前验证。
...
| 当前实现（批量当前页、合并 projects 请求） | 5 | 5 | 31 | 99.3% |

当前实现的完整刷新 SQL 数量基本不随 Workflow 总量增长；到 1,000 条时因 ORM 预加载分批，从 31 条轻微增加为 36 条：
