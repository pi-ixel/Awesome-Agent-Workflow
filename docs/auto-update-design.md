...
`start` 的版本查询是命令进入后的第一项操作，不是一个独立的“未创建
workflow”步骤。`aaw update` 复用同一套查询、下载和事务换入逻辑，
区别只在于失败处理和更新成功后是否重新执行 `start`。

```
0. `start` 进入后立即 GET /api/v1/client/release；`aaw update` 也执行同样的实时查询：
...
1. 确认有新版后执行守卫与现场检查（基于 §4.3 的自定位）：
   ...
   - 打开 skills 根目录下 `.aaw-update.lock`，以非阻塞方式获取安装级内核
     ...
     在执行。自动更新模式下获锁失败时，`start` 警告后使用当前版本继续；
     `aaw update` 则报错退出。锁文件写入 PID、开始时间、随机 owner token 和事务 ID，但
...
5. 对 zip 中所有 skill 执行一个完整更新事务（详见 §5）：
   ...
   - 提交前验证任一失败均整体回滚；全部验证成功后才标记 committed，
     然后删除 backup。
6. 更新失败且整体回滚成功：
   - 自动更新模式：向 stderr 输出 warning，使用当前旧版本继续 `start`；
   - `aaw update`：报错退出。
   如果回滚未成功，两种模式都必须立即报错退出，不允许继续创建 workflow。
7. 更新成功：
   - `aaw update`：向 stdout 打印"更新完成: 1.1.0 -> 1.2.0"并退出；
   - 自动更新模式：不在已导入旧模块的进程中继续 start。保留原始 argv，
     在 skills 根目录生成 `.aaw-update-handoff-<random>.json` 一次性 handoff 文件，
     然后以 `os.execv(sys.executable, ...)` 重新执行换入后的 `scripts/aaw.py`
     和原始 `start` argv。
```
...
- 不满足上述契约的版本必须采用新的主版本或独立迁移流程，并要求用户
  重启 Agent/harness 会话。
...
| 阶段 | 典型失败 | 现场状态 | 处置 |
|---|---|---|---|
| 获取安装级锁 | 另一更新或恢复进程持有内核锁 | **现有安装未被触碰** | 不等待也不抢占活锁；自动模式警告后继续 start，手动 update 报错 |
| 持锁后发现残留事务 | 上一 owner 异常退出 | 依事务阶段而定 | 先按事务清单恢复或清理，完成后才开新事务 |
| 下载 / 解压 / 结构检查 | 网络中断、坏 zip、磁盘满、sanity 失败 | **现有安装未被触碰** | 删除临时文件与事务目录；自动模式警告后继续 start，手动 update 报错 |
...
| 3 | `start` 自动更新 + `aaw update` | `cli/update.py`、`cli/main.py`、`_cli_base.py`、`test_cli_auto_update.py`、`test_cli_update.py` |
