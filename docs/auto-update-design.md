...
3. `aaw status` 每次进入后立即查询 latest，有新版时在读取 workflow 前
...
| 更新触发 | — | ✅ 每次 `aaw status` 自动检查；`aaw update` 可手动触发 | — |
...
- zip 根必须包含 `release-manifest.json`：
  ...
  明确从 AAW 托管集合中删除的历史 Skill。客户端只接受整数值且精确等于
  `1` 的 `schema`（布尔值不视为整数 schema）；三个列表均为必填项，且不得
  重复或交叉。未知 schema 必须在触碰现有安装前拒绝，不能按相近结构猜测解析。
...
客户端把该接口视为严格协议，而不是宽松配置：响应必须是 JSON object 且必须
包含 `latest_version`；只有显式的 `{"latest_version": null}` 表示当前无发布。
有发布时，`latest_version` 必须是严格三段版本，`file_name` 必须精确等于
`aaw-skills-{latest_version}.zip`，`size_bytes` 必须是大于 0 的 JSON integer
（布尔值不接受）。字段缺失、类型错误或文件名不匹配均按非法响应处理：自动模式
告警后继续使用本地版本，手动 `aaw update` 报错退出。
...
- **不使用 24h 或任何本地版本查询缓存**。每次执行 `aaw status`，
...
- 同机多 harness 安装（如 `~/.claude/skills/` 与 `~/.config/opencode/skills/`
  ...
  另一份在它自己执行 `status` 或 `aaw update` 时单独更新。
...
### 4.4 `aaw status` 自动更新与 `aaw update`

`status` 进入后先做必要的本地安装一致性恢复；在干净现场上，第一项
...
0. `scripts/aaw.py` 在导入 CLI 模块前已获取安装级共享锁。所有命令先做
   本地一致性预检：
   - 在共享锁下扫描 skills 根目录下所有名称严格匹配 `.aaw-txn-<id>/` 的
     残留目录，而不是只扫描能成功解析 `transaction.json` 的目录：
       committed                    → 尽力清理 backup/事务目录
       未 committed、尚未移动正式目录 → 现场未被动过，删除事务目录
       未 committed、已进 backup/换入 → 先执行内置恢复
       无 transaction.json          → 视为尚未影响正式目录的清理残留并尽力删除
   - 如果发现残留，释放共享锁并在 30 秒内获取排他锁；获锁后重新扫描，
     只对仍存在的事务恢复/清理，然后重新获取共享锁再继续。
     未 committed 事务恢复失败必须报错退出，不得查询 server 或继续 start；
     committed 或无清单的纯清理残留删除失败只输出 warning，不阻断已经一致的
     正式安装，留待下次启动重试。
   - 共享或排他锁在 30 秒内无法获取时，不得使用可能正在被替换的
     definitions 继续，任何命令都报错退出。`aaw update --json` 在 CLI 模块
     尚未导入时发生的锁超时也必须由启动器输出单个 `status=failed` JSON；
     残留恢复失败则输出 `status=recovery_required`，不能退化为非 JSON 文本。
1. `status` 立即 GET /api/v1/client/release；`aaw update` 也执行同样的实时查询：
   - 查询成功且 latest 为空或 latest ≤ 本地 VERSION → `status` 继续正常业务逻辑；
     `aaw update` 打印"已是最新"并退出；
   - 查询成功且 latest > 本地 VERSION → 继续下载和事务更新；
   - 查询超时、server 不可用或响应非法 → `status` 向 stderr 输出 warning 并
     使用当前本地版本继续；`aaw update` 报错退出。
   - 查询请求包含连接、发送和读取响应在内的总 deadline 为 30 秒，不自动重试；
     响应体最多读取 64 KiB，超过上限按非法响应处理。
2. 确认有新版后，在当前共享锁下执行基于 §4.3 自定位的初步守卫：
   ...
   - 链接守卫或其他换入前检查失败时，自动更新模式只向
     stderr 输出 warning 并使用当前版本继续 `status`；`aaw update` 则报错退出。
     已经开始移动正式目录后，必须先按 §5 完成回滚才能决定是否继续。
3. 使用响应中的 `latest_version` 和 `file_name` 构造
   `/api/v1/client/releases/{version}/download/{file_name}`。服务端文件名只参与
   URL 构造；本地始终保存为本进程 stage 内的固定文件 `release.zip`，不得把
   响应文件名拼入本地路径。将 zip 流式下载到该临时文件。
...
5. sanity 检查：
   ...
   - 对 manifest `skills` 和 `removed_skills` 指向的每个已存在目标目录单独
     做 `lstat`；任一目标是 symlink、junction、directory reparse point，或是
     普通文件等非目录对象，均拒绝整个更新且不得移动/删除该对象。
6. 换入前从共享锁升级为排他锁：释放共享锁，在 30 秒内获取排他锁，
   然后重新扫描残留事务并重读本地 VERSION。如果另一进程已经更新到
   相同或更高版本，删除本进程精确的 `.aaw-stage-<id>`，通过 handoff 重新执行当前版本的
   `status`；`aaw update` 返回已是最新。不允许已导入旧模块的进程直接继续 status。
7. 在排他锁下再次校验 manifest 全部已存在目标的 `lstat` 和冲突状态；
   ...
   - transaction schema 3 为每个目标记录 `operation`（`add` / `replace` /
     `remove`）和 `had_original`，恢复不能仅根据 backup 是否存在推断；
   - 任一步失败时，按逆序移走已换入的新 skill，再恢复全部旧 skill。对于
     `add` 目标，即使它没有 backup，只要 payload 已离开事务目录且正式位置
     已出现，也必须移走该新增 Skill，不能遗留半次更新；
   - 全部换入后，从正式位置重新读取 VERSION，确认其等于
     `latest_version`，并确认事务清单中的所有 skill 和必需入口均存在；
   - 提交前验证任一失败均整体回滚；全部验证成功后才标记 committed。
     committed 之后正式安装已经成功，删除 backup/事务目录是尽力清理：失败
     只输出 warning，更新仍返回成功，并由下次命令再次清理残留。
8. 更新失败且整体回滚成功：
...
旧版 Python 模块，不能在换入新文件后继续执行 `status`；handoff 只保存
目标版本、创建时间和随机 token，原始 `status` argv 直接作为 `execv` 参数传递，
不写入文件。新进程消费 handoff 后就知道
“文件已换入，本次不再查询 server，直接执行原 start”，从而避免循环更新。

handoff 文件必须由旧进程用排他创建生成，名称严格为
`.aaw-handoff-<16位小写十六进制>.json`，并且文件的词法父目录必须精确等于
当前 skills 根目录。新进程要求路径和 token 两个环境值同时存在，先拒绝
symlink、junction、reparse point 和非普通文件，再用原子 rename 取得该文件的
唯一所有权；随后无论校验是否成功都删除已取得的副本，避免重放，再校验 schema、
目标版本并用常量时间比较 token。外部路径或伪造文件名必须在原子取得前拒绝且
不得触碰对应文件，避免 handoff 环境变量被利用来删除安装目录外的任意文件；
安装目录内合法命名但 token 不匹配的 handoff 被一次性消费后报错退出。
...
无可用包时的 `latest_version` 为 `null`。该 JSON 契约覆盖启动器获取初始共享锁、
bootstrap 恢复、查询、下载和事务更新全链路，不因错误发生在 CLI import 前而例外。
...
- 重新执行的新进程必须先消费 handoff，确认本地 VERSION 已等于或高于
  handoff 的目标版本，然后不再请求 server，直接执行原始 `status` 参数。
...
由于用户级安装可能被多个项目共享，项目 A 的 `status` 触发更新后，项目 B
...
.aaw-txn-<random>/
  ...
  recover.py             # 仅依赖标准库的独立恢复脚本
  payload/<skill>/       # 完整的新版本（来自同名 .aaw-stage 工作区）
  backup/<skill>/        # 从正式位置移入的旧版本
  displaced/<skill>/     # 回滚时从正式位置移走的新版本
```

新事务清单使用 schema 3，并显式记录每个目标原本是否存在及本次操作：

```json
{
  "schema": 3,
  "phase": "staged",
  "skills": ["aaw-workflow", "new-skill"],
  "removed_skills": ["old-skill"],
  "targets": {
    "aaw-workflow": {"operation": "replace", "had_original": true},
    "new-skill": {"operation": "add", "had_original": false},
    "old-skill": {"operation": "remove", "had_original": true}
  },
  "steps": {}
}
```

`targets` 是回滚判断依据，尤其用于区分“没有 backup 的新增 Skill”和“尚未执行
rename”。为处理升级本功能前遗留的事务，恢复器可以兼容 schema 2，并结合旧
`steps`、backup、payload 和正式目录的实际位置推断；所有新事务只写 schema 3。
...
→ 标记 committed → 尽力清理 backup/事务目录 → 释放锁。在 committed 之前任一步失败
都进入整体回滚，必须恢复事务清单中的全部旧 skill，不允许保留
部分新版本；没有旧 backup 的 `add` 目标也必须根据 schema 3 记录移走已落地的新目录。
committed 之后的清理不再影响更新结果，清理失败只留下可重试残留并输出 warning。
...
| 提交后清理 backup | 杀软扫描等瞬时占用 | 更新已成功，仅残留事务目录 | 输出 warning 但仍返回更新成功；下次任一 CLI 命令启动时按 committed 状态重试清理 |
...
4. 中断后运行 `python <transaction-dir>/recover.py`：
   - 未 committed → 移走已换入的新 skill（包括没有 backup 的新增 Skill），
     按逆序恢复所有 backup；
...
| 3 | `status` 自动更新 + `aaw update` + 安装级读写锁 + YAML 扩展目录 | `scripts/aaw.py`、`cli/update.py`、`cli/main.py`、`cli/workflow.py`、`_cli_base.py`、`test_cli_auto_update.py`、`test_cli_update.py`、`test_cli_definition_extensions.py` |
